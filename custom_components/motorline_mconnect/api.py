    ...
    async def async_command(
        ...
        async def _post_command(body: dict) -> None:
            ...
            try:
                ...
                raise MConnectCommError(
                    f"Command {action} connection error: {e}"
                ) from e
                ...
                async with self._session.post(
                    ...
                    if resp.status == 401:
                        ...
                        raise MConnectAuthError(
                            f"Failed to exchange home token: {resp.status} {text}"
                        )
                    if resp.status == 429:
                        ...
                        if i < attempts - 1:
                            ...
                            continue
                        raise MConnectRateLimitError(
                            "Rate limited during home token exchange", retry_after=wait
                        )
                    if 500 <= resp.status < 600:
                ...
                async with self._session.post(
                    ...
                    if resp.status == 429:
                        ...
                        if i < attempts - 1:
                            ...
                            continue
                        raise MConnectRateLimitError(
                            "Rate limited during user refresh", retry_after=wait
                        )
                    if 500 <= resp.status < 600:
                        if i < attempts - 1:
                            await asyncio.sleep(1.5 * (i + 1))
                ...
                async with self._session.post(
                    ...
                    if resp.status == 429:
                        ...
                        if i < attempts - 1:
                            ...
                            continue
                        raise MConnectRateLimitError(
                            "Rate limited during home refresh", retry_after=wait
                        )
                    if 500 <= resp.status < 600:
                        if i < attempts - 1:
                            await asyncio.sleep(1.5 * (i + 1))
...
def resolve_onoff_from_values(values) -> tuple[bool, Status] | None:
    """
    Prefer values.types.OnOff 'value' (0/1).
    Returns (is_on, "1"/"0") or None if not present/parseable.
    """
    for v in values or []:
        ...
        if (
            ...
            try:
                ...
                return None
            return (is_on, "1" if is_on else "0")
    return None
