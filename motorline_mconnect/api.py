...
class MConnectClient:
    ...
    async def async_begin_login(self, username: str, password: str) -> None:
        ...
        try:
            async with self._session.post(url, json=payload, headers=headers, timeout=ClientTimeout(total=15)) as resp:
    ...
    async def async_complete_login_with_mailbox(self, provider: str, oauth_tokens: dict) -> dict:
        ...
        async def _poll_for_code(kind: str) -> str:
            ...
            while time.monotonic() < deadline:
                ...
                body, subject, sender, received_ts = await self._fetch_latest_otp_email(provider, oauth_tokens)
                if body:
                    subj = (subject or "").lower()
                    sndr = (sender or "").lower()
                    fresh_enough = (received_ts or 0) >= min_ts and (time.time() - (received_ts or 0)) <= 5 * 60
            ...
            raise MConnectAuthError(f"Timed out waiting for {kind.replace('_',' ')} email (5 minutes).")
        ...
        try:
            async with self._session.post(VERIFY_URL, json=payload, headers=headers_api, timeout=ClientTimeout(total=15)) as resp:
                text = await resp.text()
                ...
                if resp.status == 200:
                    ...
                    home_tokens = await self.async_exchange_home_token(user_access, home_id)
                ...
                if resp.status == 403 and "MaxTrustedDevicesError" in (text or ""):
                    async with self._session.get(RESET_URL, headers=headers_api, timeout=ClientTimeout(total=15)) as r2:
                        if r2.status != 200:
                            raise MConnectAuthError(f"Reset trigger failed: {r2.status} {await r2.text()}")

                    reset_code = await _poll_for_code("end_all")

                    url_del = RESET_CODE_URL.format(code=reset_code)
                    async with self._session.delete(url_del, headers=headers_api, timeout=ClientTimeout(total=15)) as r3:
                        if r3.status != 200:
                            raise MConnectAuthError(f"Reset confirm failed: {r3.status} {await r3.text()}")

                    async with self._session.delete(REMOVE_THIS_DEVICE_URL, headers=headers_api, timeout=ClientTimeout(total=15)) as r4:
                        if r4.status != 200:
                            raise MConnectAuthError(f"Trusted device delete failed: {r4.status} {await r4.text()}")

                    if not getattr(self, "_last_username", None) or not getattr(self, "_last_password", None):
                        raise MConnectAuthError("Cannot re-login: missing stored credentials.")
                    await self.async_begin_login(self._last_username, self._last_password)

                    verif_code = await _poll_for_code("verification")
                    payload = {"code": verif_code, "platform": "Win32", "model": "edge"}
                    async with self._session.post(VERIFY_URL, json=payload, headers=headers_api, timeout=ClientTimeout(total=15)) as r5:
                        t5 = await r5.text()
                        if r5.status != 200:
                            raise MConnectAuthError(f"MFA after reset failed: {r5.status} {t5}")
                        data = await r5.json()
                        user_access = data.get("access_token")
                        user_refresh = data.get("refresh_token")
    ...
    async def _mail_list_messages(self, provider: str, oauth_tokens: dict) -> list[dict]:
        """
        ...
        """
        access_token = oauth_tokens.get("token", {}).get("access_token") or oauth_tokens.get("access_token")
        if not access_token:
            return []
        ...
        if provider.endswith("_gmail"):
            ...
            async with self._session.get(url, headers=auth, params=params, timeout=ClientTimeout(total=15)) as resp:
                data = await resp.json()
            msgs = []
            for m in (data.get("messages") or []):
                ...
                if mid:
                    ...
                    msgs.append({"id": mid, "subject": "", "from": "", "body_preview": "", "received_ts": 0, "has_full": False})
            return msgs
        ...
        if provider.endswith("_microsoft"):
            ...
            async with self._session.get(url, headers=auth, timeout=ClientTimeout(total=15)) as resp:
                data = await resp.json()
            msgs = []
            for item in (data.get("value") or []):
                subj = item.get("subject") or ""
                sender = (item.get("from", {}) or {}).get("emailAddress", {}).get("address") or ""
        ...
        return []
    async def _mail_get_message(self, provider: str, oauth_tokens: dict, msg_stub: dict) -> tuple[str, str, str, int]:
        """
        ...
        if provider.endswith("_gmail"):
            ...
            for h in (data.get("payload", {}) or {}).get("headers", []):
                ...
                if n == "subject":
                    ...
                    sender = v.split("<")[-1].split(">")[0].strip() if "<" in v else v.strip()
    ...
    async def async_refresh_tokens(self, tokens: dict) -> dict:
        if not tokens or not tokens.get("refresh"):
            return tokens
        try:
            async with self._session.post(USER_REFRESH_URL, json={"refresh_token": tokens["refresh"]}, timeout=ClientTimeout(total=15)) as resp:
                if resp.status == 401:
    ...
    async def async_fetch_all(self, tokens: dict) -> dict:
        ...
        try:
            async with self._session.get(ALL_DEVICES_URL, headers=headers, timeout=ClientTimeout(total=20)) as resp:
                if resp.status == 401:
                ...
                if resp.status != 200:
                    raise MConnectCommError(f"Unexpected fetch status {resp.status}: {await resp.text()}")
                ...
                try:
                    ...
                    text = await resp.text()
                    raise MConnectCommError(f"Invalid JSON from fetch: {text[:200]}") from e
    ...
    async def _fetch_latest_otp_email(self, provider: str, oauth_tokens: dict) -> tuple[str, str, str, int]:
        """
    ...
    for stub in stubs:
        body, subject, sender, received_ts = await self._mail_get_message(provider, oauth_tokens, stub)
    ...
    async def async_get_homes(self, access_token: str) -> list[dict]:
        ...
        async with self._session.get(HOMES_URL, headers=headers, timeout=ClientTimeout(total=15)) as resp:
            if resp.status != 200:
                raise MConnectCommError(f"Failed to fetch homes: {resp.status} {await resp.text()}")
    ...
    async def async_exchange_home_token(self, access_token: str, home_id: str) -> dict:
        ...
        async with self._session.post(HOMES_TOKEN_URL, json=payload, headers=headers, timeout=ClientTimeout(total=15)) as resp:
            if resp.status != 200:
                raise MConnectCommError(f"Failed to exchange home token: {resp.status} {await resp.text()}")
            data = await resp.json()
            return {
                "access": data.get("access_token"),
    ...
    async def async_refresh_user_tokens(self, refresh_token: str) -> dict:
        ...
        async with self._session.post(USER_REFRESH_URL, json=payload, headers=headers, timeout=ClientTimeout(total=15)) as resp:
            text = await resp.text()
            if resp.status != 200:
    ...
    async def async_refresh_home_tokens(self, home_id: str, refresh_token: str) -> dict:
        ...
        async with self._session.post(HOME_REFRESH_URL, json=payload, headers=headers, timeout=ClientTimeout(total=15)) as resp:
            text = await resp.text()
            if resp.status != 200:
...
def _parse_iso_to_epoch(iso_str: str | None) -> int:
    ...
    try:
        ...
        return int(datetime.fromisoformat(iso_str.replace("Z", "+00:00")).replace(tzinfo=timezone.utc).timestamp())
    except Exception:
        return 0
