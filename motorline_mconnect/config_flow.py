...
from .api import MConnectClient, MConnectAuthError, MConnectCommError

_LOGGER = logging.getLogger(__name__)
_LOGGER.warning("MMCONNECT: config_flow import started")
# --- prove module import (should appear in Logs) ---
_LOGGER.debug("motorline_mconnect.config_flow: module import started")


from typing import Any
from homeassistant.helpers import config_entry_oauth2_flow
from homeassistant.config_entries import HANDLERS as _FLOW_HANDLERS
...
class ConfigFlow(config_entry_oauth2_flow.AbstractOAuth2FlowHandler, domain=DOMAIN):
    ...
    async def async_step_oauth(self, user_input: dict | None = None) -> ConfigFlowResult:
        ...
        return self.async_show_form(
            step_id="oauth_setup",
            data_schema=vol.Schema({vol.Required("setup_complete", default=False): bool}),
    ...
    async def async_step_oauth_setup(self, user_input: dict | None = None) -> ConfigFlowResult:
        """Confirm and hand off to the OAuth2 helper."""
    ...
    async def async_step_pick_implementation(self, user_input: dict | None = None) -> ConfigFlowResult:
        """Delegate to the base helper (builds implementation from Application Credentials)."""
        return await super().async_step_pick_implementation(user_input)

    async def async_step_auth(self, user_input: dict | None = None) -> ConfigFlowResult:
        """Start the OAuth authorization using HA's helper."""
        return await super().async_step_auth(user_input)

    async def async_oauth_create_entry(self, data: dict) -> ConfigFlowResult:
        """Receive OAuth2 tokens and finish login."""
        _LOGGER.debug("OAuth callback keys: %s", list(data.keys()))
            ...
            if stored_mconnect and stored_mconnect.get("refresh_token"):
                ...
                try:
                    ...
                    if home_id and refresh_token:
                        refreshed = await client.async_refresh_home_tokens(home_id, refresh_token)
                        new_data = dict(self._reauth_entry.data)
                        new_data[CONF_MCONNECT_TOKENS] = refreshed
                        return self.async_update_reload_and_abort(self._reauth_entry, data=new_data)
                except Exception:  # allow fallthrough to full reauth
                    pass
    ...
    async def _finish_login(self) -> ConfigFlowResult:
        ...
        try:
            ...
            return self.async_show_form(
                step_id="user", errors={"base": "invalid_auth"}, description_placeholders={"error": str(e)}
            )
        except MConnectCommError as e:
            return self.async_show_form(
                step_id="user", errors={"base": "cannot_connect"}, description_placeholders={"error": str(e)}
            )
        except Exception as e:  # noqa: BLE001
            _LOGGER.exception("Unexpected error in _finish_login: %s", e)
            return self.async_show_form(
                step_id="user", errors={"base": "unknown"}, description_placeholders={"error": str(e)}
            )
...
# --- prove class exists after import (should appear in Logs) ---
try:
    _LOGGER.debug(
...
try:
    ok = issubclass(ConfigFlow, config_entry_oauth2_flow.AbstractOAuth2FlowHandler)
    dom = getattr(ConfigFlow, "domain", None)
    _LOGGER.warning("MMCONNECT: class discovered=%s domain=%r", ok, dom)
except Exception as e:
    _LOGGER.exception("MMCONNECT: class discovery failed: %s", e)

_LOGGER.warning("MMCONNECT: in HANDLERS=%s keys=%s",
                "motorline_mconnect" in _FLOW_HANDLERS,
                list(_FLOW_HANDLERS.keys())[:8])
...
# try:
#     # Prove the class is defined and is a subclass of the expected base
#     _ = issubclass  # avoid NameError on odd environments
# except Exception:
#     pass

# class ConfigFlow(config_entry_oauth2_flow.AbstractOAuth2FlowHandler):
#     domain = DOMAIN          # <- required as a class attribute
#     VERSION = 1
